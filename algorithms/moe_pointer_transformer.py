...
class MoePointerTransformer(nn.Module, IterMixin):

    def __init__(self, n_enc_bloc, n_dec_block, n_embd, n_head, rel_dim, device, max_len=1100, env_args=None, use_ar=True, use_relation=True, use_node_emb=False, use_unbind_decode=False, only_heuristic=False, use_moe=True, hypers=None):
        super().__init__()
        ...
        self.other_node_prob = hypers.get("other_node_prob", 0.00)
        self.no_assign_prob = hypers.get("no_assign_prob", 0.00)
        self.load_balance_weight = hypers.get("load_balance_weight", 0.5)
    ...
    def forward(self, obs, input_actions=None, deterministic=False, only_critic=False):  
        ...
        for _idx in range(n_courier):
            ...
            prior_guided_mask = torch.where(node_have_request_to_pickup | node_is_potential_station1 | node_have_request_to, 0.1, self.other_node_prob)
