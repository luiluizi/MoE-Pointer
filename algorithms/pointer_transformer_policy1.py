...
class PointerTransformerPolicy:
    ...
    def __init__(self, args, env_args, device, only_heuristic=False):
        ...
        if self.algorithm in ["mapt", "prob_heuristic"]:
            from algorithms.transfer_pointer_transformer import MultiAgentPointerTransformer
            # from algorithms.transfer_pointer_transformer_new_design import MultiAgentPointerTransformer
            self.transformer = MultiAgentPointerTransformer(
                ...
                use_ar=not args.not_use_ar, use_heur_req=not args.not_use_heur_req, use_heur_veh=not args.not_use_heur_veh, use_relation=not args.not_use_relation, use_tsp=not args.not_use_tsp, 
                ...
                hypers={"other_node_prob": args.other_node_prob, "no_assign_prob": args.no_assign_prob, "load_balance_weight": args.load_balance_weight, "concentrat_weight": args.concentrat_weight}
            )
    ...
    def get_actions(self, obs, rnn_states_actor, rnn_states_critic, deterministic=False):
        ...
        actions, action_log_probs, entropy, values, aux_losses = self.transformer.forward(obs, None, deterministic=deterministic, heuristic_weight=self.heuristic_weight)
    ...
    def evaluate_actions(self, obs, rnn_states_actor, rnn_states_critic, actions):
        ...
        actions, action_log_probs, entropy, values, aux_losses = self.transformer.forward(obs, actions, heuristic_weight=self.heuristic_weight)

        return values, action_log_probs, entropy, aux_losses
