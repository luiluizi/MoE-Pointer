...
class DroneTransferEnv(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        # self.couriers["time_left"][c_dispatch_mask] = self._global["node_node"][batch_idx, self.requests["station1"][c_dispatch_mask], courier_to[c_dispatch_mask]] 
        # self.couriers["target"][c_dispatch_mask] = courier_to[c_dispatch_mask]

        # # 惩罚指派station距离起始和目标点距离过远的指派
        # request_station1_penalty = torch.zeros(self.batch_size, dtype=torch.int64, device=self.device)
        # request_station2_penalty = torch.zeros(self.batch_size, dtype=torch.int64, device=self.device)
        ...
        if not (self._global["drone_request"][batch_idx, :, request_idx]==0).all():
            print(self._global["drone_request"][batch_idx, :, request_idx])
            print(self._global["drone_request"][batch_idx, :, request_idx].shape)
            print(request_idx)
        assert (self._global["drone_request"][batch_idx, :, request_idx]==0).all(), "double check, only `no_relation` requests should be assigned."
        ...
        # assert((drone_to[d_dispatch_mask] != self.n_node).all())
        # assert((self.drones["target"][d_dispatch_mask] != self.n_node).all())
        ...
        self.couriers["value"] += current_value_stage1.sum(dim=-1) # 每个courier的累积价值
        ...
        self.couriers["value"] += current_value_stage3.sum(dim=-1)
        ...
        # rewards = current_value_stage1 +  current_value_stage2 +  current_value_stage3 - 10 * add_dist_courier * self.courier_cost_ratio - 10 * add_dist_drone * self.drone_cost_ratio
        # rewards = 0
        # rewards = current_value_stage3 - current_cost_courier * self.courier_cost_ratio - current_cost_drone * self.drone_cost_ratio
        # rewards = 0.1 * current_value_stage1 + 0.2 * current_value_stage2 + 0.7 * current_value_stage3 - current_cost * self.dist_cost
        # rewards = current_value_stage3 - current_cost * self.dist_cost
        # 检查batch中是否某个环境已经结束
